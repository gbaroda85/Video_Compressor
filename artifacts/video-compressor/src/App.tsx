  ...
  return(
    <div onDrop={e=>{e.preventDefault();setDrag(false);const f=e.dataTransfer.files[0];if(f?.type.startsWith("video/"))onFile(f);}}
      ...
      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all ${drag?"bg-purple-500/20 scale-110":"bg-[var(--surface-2)] group-hover:bg-purple-500/10"}`}>
        <svg className={`w-7 h-7 transition-colors ${drag?"text-purple-500":"text-[var(--text-4)] group-hover:text-purple-500"}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return(
    <div className="space-y-4">
      ...
      <div className="flex gap-2.5">
        ...
        <button onClick={onRedo} className="px-5 py-3.5 rounded-xl border border-[var(--border-2)] text-sm font-semibold text-[var(--text-3)] hover:text-[var(--text)] hover:border-[var(--border-2)] transition-all">Try again</button>
      </div>
    </div>
  );
}
...
// ── Reusable hook (audio/mute/split) ──────────────────────────────────────────
        ...
        pollRef.current=setInterval(async()=>{
          try{
            ...
            if(data.status==="done"){
              ...
              dl.onload=()=>{
                if(dl.status===200){
                  const blob=new Blob([dl.response],{type:"video/mp4"});
                  setResult({url:URL.createObjectURL(blob),size:Number(dl.getResponseHeader("X-Compressed-Size"))||blob.size,originalSize:Number(dl.getResponseHeader("X-Original-Size"))||info.size,ext:"mp4",mimeType:"video/mp4"});
                  setStage("done");
  ...
  return(
    <div className="space-y-5">
      ...
      {busy&&<>
        ...
        <button onClick={()=>{xhrRef.current?.abort();stopPoll();setStage("ready");}} className="w-full text-center text-xs text-[var(--text-4)] hover:text-[var(--text-3)] transition-colors underline underline-offset-2">Cancel</button>
      </>}
...
function SplitTool(){
  ...
  return(
    <div className="space-y-5">
      ...
      {busy&&<>{stage==="uploading"&&<UploadProgress progress={progress}/>}{stage==="processing"&&<Spinner/>}<button onClick={()=>{xhrRef.current?.abort();setStage("ready");}} className="w-full text-center text-xs text-[var(--text-4)] hover:text-[var(--text-3)] transition-colors underline underline-offset-2">Cancel</button></>}
...
function SimpleToolUI({endpoint,mime,ext,buttonLabel,outSuffix}:{endpoint:string;mime:string;ext:string;buttonLabel:string;outSuffix:string}){
  ...
  return(
    <div className="space-y-5">
      ...
      {busy&&<>{t.stage==="uploading"&&<UploadProgress progress={t.progress}/>}{t.stage==="processing"&&<Spinner/>}<button onClick={t.cancel} className="w-full text-center text-xs text-[var(--text-4)] hover:text-[var(--text-3)] transition-colors underline underline-offset-2">Cancel</button></>}
  ...
  return(
    <div className="min-h-screen flex flex-col" style={{background:"var(--bg)"}}>
      ...
      <section className="relative z-10 pt-16 pb-14 text-center px-4">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-purple-500/20 bg-purple-500/6 text-purple-500 text-xs font-medium mb-6">
        ...
        <p className="text-[var(--text-3)] text-base sm:text-lg max-w-md mx-auto leading-relaxed">
          Compress, trim, extract audio — all processed instantly.<br className="hidden sm:block"/> No sign-up, no limits.
        </p>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {TOOLS.map(tool=>(
            <button key={tool.id} onClick={()=>setActive(tool.id)}
              className="group text-left p-6 rounded-2xl border border-[var(--border)] bg-[var(--surface)] hover:bg-[var(--surface-2)] hover:border-[var(--border-2)] transition-all duration-200 cursor-pointer">
              <div className="flex items-start gap-4">
                <div className={`w-11 h-11 rounded-xl flex-shrink-0 flex items-center justify-center bg-gradient-to-br ${tool.color} shadow-lg`}>
                  {tool.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <h2 className="font-bold text-[15px] text-[var(--text)] group-hover:text-purple-500 transition-colors">{tool.title}</h2>
                    <svg className="w-4 h-4 text-[var(--text-4)] group-hover:text-purple-400 group-hover:translate-x-0.5 transition-all flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                  </div>
                  <p className="text-[13px] text-[var(--text-3)] mt-1 leading-relaxed">{tool.desc}</p>
                </div>
              </div>
            </button>
          ))}
